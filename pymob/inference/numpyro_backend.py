...
class NumpyroBackend:
    ...
    def model(self, solver, obs):
        # k_i = numpyro.sample("k_i", dist.LogNormal(loc=jnp.log(5), scale=2))
        k_i = numpyro.sample("k_i", dist.Normal(loc=5, scale=10))
        # r_rt = numpyro.sample("r_rt", dist.LogNormal(loc=jnp.log(0.1), scale=s))
        # r_rd = numpyro.sample("r_rd", dist.LogNormal(loc=jnp.log(0.5), scale=s))
        # v_rt = numpyro.sample("v_rt", dist.LogNormal(loc=jnp.log(1.0), scale=s))
        # z_ci = numpyro.sample("z_ci", dist.LogNormal(loc=jnp.log(500.0), scale=s))
        ...
        sim_cint_i = sim[1][0,:,0]
        obs_cint_i = obs[1][0,:,0]
    ...
    def run(self):
        from mod import tktd_rna_3

        # @jax.jit
        # def solver(y0, params, meas_times):
        ...
        y_sim = self.evaluator(self.simulation.model_parameter_dict)
        ...
        # model = partial(self.model, solver=solver, obs=obs, mask=mask, nzfe=nzfe)    
        ...
        kernel = infer.NUTS(
            ...
            init_strategy=infer.init_to_value(values={"k_i": 5.0})
        )
        ...
        mcmc.run(jax.random.PRNGKey(123), obs=y_sim)
        mcmc.print_summary()
