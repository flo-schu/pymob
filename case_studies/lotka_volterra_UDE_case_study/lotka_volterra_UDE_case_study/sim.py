...
from case_studies.lotka_volterra_UDE_case_study.lotka_volterra_UDE_case_study.mod import lotka_volterra_UDE, Func, transformWeightsBackwards, transformBiasBackwards, setFuncWeightsAndBias
...
class UDESimulation(Simulation_v2):
    ...
    model = Func(10,10,key=model_key,theta_true=jnp.array([alpha,beta,gamma]))
    ...
    def initialize(self, input):
        ...
        key = jr.PRNGKey(5678)
        data_key, model_key, loader_key = jr.split(key, 3)
        self.model = Func(10,10,key=model_key,theta_true=jnp.array([alpha,beta,gamma]))
