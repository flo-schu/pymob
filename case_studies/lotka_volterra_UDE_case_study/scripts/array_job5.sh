...
echo "running SINDy3.py with length=$length, lr=$lr, clip=$clip, batch=$batch, points=$points, and noise=$noise"

# Run simulation
python3 SINDy3.py -length=$length -lr=$lr -clip=$clip -batch=$batch -points=$points -noise=$noise