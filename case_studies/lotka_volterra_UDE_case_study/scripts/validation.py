...
def main(length_strategy, lr_strategy, clip_strategy, batch_size, data_points, data_noise):
    ...
    try:
        ...
        for time_series in jnp.arange(0,10):
            ...
            fig.tight_layout()
            fig.savefig(f"hyperparams/scenario_{str(data_points)}_{str(data_noise)}_hyperparams_{str(length_strategy)}_{str(lr_strategy)}_{str(clip_strategy)}_{str(batch_size)}/validation_{str(time_series)}.png")
        ...
        for time_series in jnp.arange(0,10):
            ...
            fig.tight_layout()
            fig.savefig(f"hyperparams/scenario_{str(data_points)}_{str(data_noise)}_hyperparams_{str(length_strategy)}_{str(lr_strategy)}_{str(clip_strategy)}_{str(batch_size)}/validation_extrapolation_{str(time_series)}.png")

        # Save index and loss of the best model
        with open(f"hyperparams/scenario_{str(data_points)}_{str(data_noise)}_hyperparams_{str(length_strategy)}_{str(lr_strategy)}_{str(clip_strategy)}_{str(batch_size)}/validation_results.txt","w") as variable_name:
