...
def main(length_strategy, lr_strategy, clip_strategy, batch_size, data_points, data_noise):
        ...
        for (i,m) in enumerate(idata.posterior.draw.values):
            ...
            for j in range(10):
                ...
                try:
                    ...
                    success1 = ((re.search("\+\s\d+\.\d+\sX", eqs[0]) != None) or (re.match("\d+\.\d+\sX", eqs[0]) == None)) and (re.search("\-\d+\.\d+\sX\sY", eqs[0]) != None)
                    success2 = ((re.search("\+\s\d+\.\d+\sX\sY", eqs[1]) != None) or (re.match("\d+\.\d+\sX\sY", eqs[1]) == None)) and (re.search("\-\d+\.\d+\sY", eqs[1]) != None)
    ...
    with open(f"hyperparams/scenario_{str(data_points)}_{str(data_noise)}_hyperparams_{str(length_strategy)}_{str(lr_strategy)}_{str(clip_strategy)}_{str(batch_size)}/SINDy_results3.txt","w") as variable_name:
