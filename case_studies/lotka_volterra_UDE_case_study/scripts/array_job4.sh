#!/bin/bash
#SBATCH --job-name=UDE_eval_SINDy
...
echo "running SINDy2.py with length=$length, lr=$lr, clip=$clip, batch=$batch, points=$points, and noise=$noise"

# Run simulation
python3 SINDy2.py -length=$length -lr=$lr -clip=$clip -batch=$batch -points=$points -noise=$noise